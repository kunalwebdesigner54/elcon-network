...
function AddShoppingProducts() {
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      ...
      window.alert(error?.response?.data?.message || `Unable to ${isEditMode ? 'update' : 'add'} shopping product`);
    }
  };
  ...
  return (
    <section className="panel admin-add-product-panel">
      <h2 className="section-title admin-add-product-title">{isEditMode ? 'EDIT' : 'ADD'} SHOPPING PRODUCTS</h2>
