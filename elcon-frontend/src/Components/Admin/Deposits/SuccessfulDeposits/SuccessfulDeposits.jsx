...
function SuccessfulDeposits() {
  ...
  const loadRows = async () => {
    try {
      const response = await getAdminDepositRequests('Succeed');
            ...
            <tbody>
              {loading ? (
                ...
                <tr key={`${row.sno}-${row.transactionId}`}>
                  ...
                  <td>{row.status}</td>
                  <td className="action-cell"><DepositActionButtons depositId={row.depositId} utrNumber={row.utrNumber} reloadRows={loadRows} allowedActions={[]} /></td>
