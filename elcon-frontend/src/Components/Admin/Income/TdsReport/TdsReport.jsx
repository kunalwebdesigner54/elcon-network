...
function TdsReport() {
  ...
  useEffect(() => {
    Promise.all([getMemberPerformance()])
      .then(([performanceResponse]) => {
        const performanceRows = Array.isArray(performanceResponse.data)
      ...
      .catch((loadError) =>
        setError(
          loadError?.response?.data?.message || "Failed to load TDS report.",
        ),
      )
  ...
  const totalPages = Math.max(
    1,
    Math.ceil(filteredRows.length / Number(pageSize)),
  );
  ...
  const totalGrossIncome = filteredRows
    .reduce((sum, row) => sum + Number(row.grossIncome || 0), 0)
    .toFixed(2);
  const totalNetPayable = filteredRows
    .reduce((sum, row) => sum + Number(row.netPayable || 0), 0)
    .toFixed(2);
  const totalTdsAmount = filteredRows
    .reduce((sum, row) => sum + Number(row.totalTds || 0), 0)
    .toFixed(2);
    ...
    <div className="tds-report-report-page">
      ...
      <section className="panel tds-report-panel">
        <div className="tds-report-filter-row">
          <input
            type="text"
            className="text-input tds-report-filter-input"
            placeholder="MEMBER ID"
            value={filters.memberId}
            onChange={onFilterChange("memberId")}
          />
          <input
            type="text"
            className="text-input tds-report-filter-input"
          ...
          />
          <select
            className="select-input tds-report-filter-input tds-report-size-select"
            value={pageSize}
          ...
          <button type="button" className="btn-primary tds-report-search-btn">
            Search
          </button>
        </div>

        <div className="tds-report-export-row">
          <button type="button" className="btn-outline tds-report-export-btn">
            XLS
          </button>
          <button type="button" className="btn-outline tds-report-export-btn">
            PDF
          ...
          <table className="data-table">
            ...
            <tbody>
              {loading ? (
                ...
                <tr>
                  <td colSpan={9}>No TDS records found.</td>
                </tr>
              ) : (
                tdsRows.map((row) => (
            ...
            <tfoot>
              <tr>
                ...
                <td style={{ fontWeight: "bold" }}>{totalGrossIncome}</td>
                <td style={{ fontWeight: "bold" }}>{totalNetPayable}</td>
                <td style={{ fontWeight: "bold" }}>{totalTdsAmount}</td>
              </tr>
            </tfoot>
          </table>
        </div>
        ...
        >
          <div
            style={{ fontWeight: "600", color: "#00d2ff", fontSize: "16px" }}
          >
              ...
              if (
                ...
                return (
                  <button
                    key={pageNum}
                    type="button"
                    className={`page-btn ${currentPage === pageNum ? "tds-report-active" : ""}`}
                ...
                return (
                  <span
                    key={pageNum}
                    style={{ color: "#00e5ff", padding: "0 5px" }}
                  >
                    ...
                  </span>
                );
