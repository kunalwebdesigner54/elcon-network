...
import { getDonationTarget, submitDonation, getMyDonations } from "../../../api/donationsService";
...
const GivenHelp = () => {
  ...
  const loadData = async () => {
    try {
      ...
      // Fetch my donations to determine true level and build progress table
      const myDonationsReq = await getMyDonations();
      const sentDonations = myDonationsReq.data.sent || [];
      setDonationHistory(sentDonations);
      
      const unlock = myDonationsReq.data.summary.currentUnlockLevel || 0;
      ...
      // Attempt to get target for the next level
      try {
        const target = await getDonationTarget(next);
        setTargetData(target.data);
      } catch (targetErr) {
        // If there's a pending submission or no eligible upline, it might throw
        if (targetErr?.response?.status !== 400 && targetErr?.response?.status !== 404) {
          setError(targetErr?.response?.data?.message || "Failed to load donation target details.");
        } else {
          setTargetData(null); // Just means they might have a pending one or no upline
        }
      }
    } catch (err) {
  ...
  return (
    <div className="given-help-container">
      ...
      {error && (
        <div style={{ background: "rgba(220, 53, 69, 0.1)", color: "#ff4d4d", padding: "10px 16px", borderRadius: 6, marginBottom: 12, border: "1px solid rgba(220, 53, 69, 0.2)" }}>
          {error}
        </div>
      )}
      {successMsg && (
        <div style={{ background: "rgba(40, 167, 69, 0.1)", color: "#00d084", padding: "10px 16px", borderRadius: 6, marginBottom: 12, border: "1px solid rgba(40, 167, 69, 0.2)" }}>
          {successMsg}
        </div>
      )}

      {/* Donation Form or Pending Notice */}
      {!error && (
        isCurrentlyPending ? (
      ...
      {/* Donation Level Progress Table */}
      <div className="donation-progress-container">
        <h3 className="donation-progress-title">Donation Level Progress</h3>
        <div className="table-responsive custom-scrollbar">
          <table className="donation-progress-table">
            <thead>
              <tr>
                <th>Level</th>
            ...
            <tbody>
              {[...Array(10)].map((_, i) => {
                const level = i + 1;
                // Find matching donation history for this level (prioritize most recent or APPROVED)
                const hist = donationHistory.find(d => d.level === level && (d.status === 'APPROVED' || d.status === 'COMPLETED')) || 
                             donationHistory.find(d => d.level === level && (d.status === 'WAITING_FOR_RECEIVER_CONFIRMATION' || d.status === 'PENDING'));
                ...
                return (
                  <tr key={level} className={status === 'LOCKED' ? 'row-locked' : ''}>
                    <td>Level {level}</td>
                    <td>₹{DONATION_AMOUNTS[level].toLocaleString("en-IN")}</td>
                    <td>{getStatusBadge(status)}</td>
                    <td>{hist ? hist.date : '---'}</td>
                    <td>{hist ? `${hist.toName} (${hist.toMemberId})` : '---'}</td>
                    <td>{hist?.utrNumber ? hist.utrNumber : '---'}</td>
                  </tr>
                );
              })}
            </tbody>
