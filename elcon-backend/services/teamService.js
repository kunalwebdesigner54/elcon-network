...
const getTeamStats = async (memberId) => {
  const adminMemberId = await User.findOne({ role: 'admin' }).select('memberId').then(a => a?.memberId);
    const allUsers = await User.find({ role: 'user', email: { $ne: 'admin@gmail.com' } }).select('memberId sponsorId name createdAt city accountStatus rank unlockLevel contactNo email panNo -_id').lean();
...
const getAllUsersTeamStats = async () => {
    ...
    const users = await User.find({ role: 'user', email: { $ne: 'admin@gmail.com' } }).select('memberId sponsorId name createdAt city accountStatus rank unlockLevel contactNo email panNo -_id').lean();
